...
cbuffer cbExternals
{
    ...
    float3 gSpecularColor < string UIName = "Specular Color"; > = float3(0.5, 0.5, 0.5);
    ...
    bool   gUseSpecularTexture  < string UIName = "Use Texture for Specular Color"; > = false;
    ...
    float  gEnvFresnelScale < string UIName = "Scale Fresnel from Environment"; > = 1.0;
};
...
Texture2D    SpecularTexture     : register(t1)<int mipmaplevels = 0; >;
Texture2D    NormalTexture       : register(t2)<int mipmaplevels = 0; >;
Texture2D    RMACTexture         : register(t3)<int mipmaplevels = 0;>;
TextureCube  EnvironmentTexture  : register(t4)<int mipmaplevels = 0;>;
Texture2D    EmissiveTexture     : register(t5)<int mipmaplevels = 0;>;
Texture2D    GlobalAOTexture     : register(t6)<int mipmaplevels = 0;>;
...
float4 PSMain(PS_INPUT input) : SV_Target
{
    ...
    float3 diffuseColor;
    float3 F0;
    if (gUseSpecularTexture)
    {
        diffuseColor = baseColor;
        F0 = SpecularTexture.Sample(SAMPLER0, input.UV0*btScale).rgb;
    }
    else
    {
        diffuseColor = (1 - metal)*baseColor.rgb;
        F0 = ((1 - metal)*0.04 + metal * baseColor.rgb);
    }
