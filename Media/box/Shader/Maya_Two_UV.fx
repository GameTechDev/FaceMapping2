...
float ComputeShadowAmount(PS_INPUT input)
{
    ...
    float  shadowAmount = _Shadow.SampleCmp(SHADOW_SAMPLER, lightUV.xy, lightUV.z - 0.01).r;
    return shadowAmount;
}
...
float4 PSMain(PS_INPUT input) : SV_Target
{
    ...
    float3 diffuseColor = (1 - metal)*baseColor.rgb;
        ...
        //float alpha = 2.0 / (p*len / lerp(p, 1.0, len) - 1.0);
    ...
    float2 AB = float2(-1.04, 1.04) * a004 + rr.zw;
        ...
        {
        float3 envDiff = EnvironmentTexture.SampleLevel(SAMPLER0, n*float3(1, 1, 1), MAX_LOD).rgb;
            float specularSampleLevel = EnvironmentTexture.CalculateLevelOfDetail(SAMPLER0, -r)*.8 * (1 - roughness) + MAX_LOD*roughness;
