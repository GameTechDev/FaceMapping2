...
void ImGui::NewFrame()
{
    ...
    g.IO.WantCaptureMouse = (!mouse_owned_by_application && g.HoveredWindow != NULL && (g.HoveredWindow->Flags & ImGuiWindowFlags_NoInputs) == 0) || (!mouse_owned_by_application && mouse_any_down) || (g.ActiveId != 0) || (!g.OpenedPopupStack.empty()) || (g.CaptureMouseNextFrame);
...
bool ImGui::ColorEdit3(const char* label, float col[3], bool *colorButtonClicked)
{
    float col4[4];
    col4[0] = col[0];
    col4[1] = col[1];
    col4[2] = col[2];
    col4[3] = 1.0f;
	const bool value_changed = ImGui::ColorEdit4(label, col4, false, colorButtonClicked);
    col[0] = col4[0];
    col[1] = col4[1];
    col[2] = col4[2];
    return value_changed;
...
bool ImGui::ColorEdit4(const char* label, float col[4], bool alpha, bool *colorButtonClicked)
{
	if (colorButtonClicked != NULL)
	...
	{
		ImGui::SameLine(0, style.ItemInnerSpacing.x);
		*colorButtonClicked = ImGui::Button("..", ImVec2(0, 0));
	}
